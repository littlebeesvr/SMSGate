...
public class WapSIPush implements WbxmlDocument, Serializable {
	...
	public static final String[] SI_TAG_TOKENS = {"si", // 05
			"indication", // 06
			"info", // 07
			"item", // 08
	};

	public static final String[] SI_ATTR_START_TOKENS = {"action=signal-none", // 05
	...
	public static final String[] SI_ATTR_VALUE_TOKENS = {".com/", // 85
			".edu/", // 86
			".net/", // 87
			".org/", // 88
	};
	...
	/*
	 * public static void main(String argv[]) throws Exception { SIPush push = new SIPush("http://wap.tv4.se/", "TV4 Nyheter"); push.writeTo(getWbxmlWriter(), new
	 * FileOutputStream("si.wbxml")); }
	 */

	private byte[] encodeDateTime(Date date) {
		return null;
		/*
		 * If used, the attribute value MUST be expressed in a date/time representation based on [ISO8601] as specified in [HTML4]. However, SI does not allow use of time zones;
		 * the time MUST always be expressed in Co-ordinated Universal Time (UTC), a 24-hour timekeeping system (indicated by the ?Z?). The format is: YYYY-MM-DDThh:mm:ssZ Where:
		 * YYYY = 4 digit year (?0000? ... ?9999?) MM = 2 digit month (?01?=January, ?02?=February ... ?12?=December) DD = 2 digit day (?01?, ?02? ... ?31?) hh = 2 digit hour,
		 * 24-hour timekeeping system (00 ... 23) mm = 2 digit minute (?00? ... ?59?) ss = 2 digit second (?00? ... ?59?) Note: T and Z appear literally in the string. Example:
		 * 1999-04-30T06:40:00Z means 6.40 in the morning UTC on the 30th of April 1999.
		 */
	}
