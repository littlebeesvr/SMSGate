...
/**
 * <p>
 * Tool or command class that emulates the libwbxml <em>xml2wbxml</em> counterpart. This command let the user convert a common XML file into a binary WBXML file.
 * </p>
 * 
 * <h3>NAME</h3>
 * 
 ...
 * 
 * <p>
 * Command that converts a text XML file into a WBXML binary file. It uses an intermediary form to represent the structure and then normal DOM or JAXB techniques are used. In case
 * of JAXB the representation classes should be provided (the <em>wbxml-stream</em> package does not contain any JAXB classes, some classes were created for a few languages only
 * for testing purposes but they are not packaged in the library).
 * </p>
 * 
 * <p>
 ...
 * <li>
 * <p>
 * <strong>-j --jaxb</strong>: Use JAXB processing (object representation) instead of normal DOM processing. As it was said JAXB classes are not part of the <em>wbxml-tream</em>
 * library.
 * </p>
 * </li>
 * <li>
 * <p>
 * <strong>-e --event</strong>: Use the event writer instead of common stream writer implementation.
 * </p>
 * </li>
 * <li>
 * <p>
 * <strong>-t --type</strong>: The type of use of the string table in the encoding. There are three types: <strong>IF_NEEDED</strong> (default, only use the string table if
 * needed), <strong>ALWAYS</strong> (use for all the strings) and <strong>NO</strong> (no use it at all, it can report problems if the encoding needs it).
 * </p>
 * </li>
 * <li>
 * <p>
 * <strong>-k --keep</strong>: Keep the spaces in the values (by default all the string are trimmed).
 * </p>
 * </li>
 * <li>
 * <p>
 * <strong>-d --definition</strong>: Use a fixed language definition for the WBXML file. If no one is provided the command tries to guess it using the public identifier of the
 * WBXML file. But if it is unknown (some languages are nor standardized, like Microsoft ActiveSync) and no definition is provided an error is reported.
 * </p>
 * <p>
 ...
 * <li>
 * <p>
 * <strong>infile</strong>: XML file to convert, <em>-</em> can be provided to use the standard output.</em>
 * <li>
 * <p>
 * <strong>outfile</strong>: file to write the resulting WBXML, <em>-</em> can be provided to use the standard input.</em>
 * </ul>
 * 
 * <h3>EXAMPLES</h3>
 * 
...
final public class Xml2WbXml {
	 ...
	 * 
	 * @param message
	 *            The message to show previous of the usage part
	 */
	...
	/**
	 * Method that gets the value part of a parameter if it exists. If the array has no more elements usage is called.
	 * 
	 * @param args
	...
	/**
	 * Constructor that creates the command using the arguments passed by the caller. Usage is used (IllegalArgumentException) if some error is detected in the arguments.
	 * 
	 * @param args
	 *            The argument list
	 * @throws Exception
	...
	/**
	 * Method that executes the command, ie, does the conversion from WBXML to XML.
	 * 
	 * @throws Exception
	 *             Some error
	 */
	private void process() throws Exception {
		...
		try {
			...
			if (def == null) {
				...
				if (def == null) {
					def = WbXmlInitialization.getDefinitionByRoot(element.getLocalName(), element.getNamespaceURI());
					if (def == null) {
						usage(String.format("Definition not found for name=%s and namespace=%s. Use -d argument to force it.", element.getLocalName(), element.getNamespaceURI()));
					}
				}
			}
			// create the factory
			...
			if (!useDom) {
				...
				if (clazz == null || clazz.isEmpty()) {
					usage(String.format("The definition '%s' does not contain a main class.", def.getName()));
				}
	 ...
	 * 
	 * @param args
	 *            The arguments to call the command
	 * @throws Exception
	 *             Some error in the invocation
	 */
