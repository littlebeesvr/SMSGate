...
/**
 * <p>
 * Tool or command class that emulates the libwbxml <em>wbxml2xml</em> counterpart. This command let the user convert a WBML file into a common XML file.
 * </p>
 * 
 * <h3>NAME</h3>
 * 
 * <p>
 * wbXml2Xml - Convert a WBXML binary file into a common XML text file
 * </p>
 * 
 * <h3>SYNOPSYS</h3>
 * 
 * <p>
 * java -cp wbxml-jaxb-X.X.X.jar es.rickyepoderi.wbxml.tools.Xml2WbXml [-j --jaxb] [-d --definition $lt;NAME&gt;] {infile} {outfile}
 * </p>
 * 
 * <h3>DESCRIPTION</h3>
 * 
 * <p>
 * Command that converts a binary WBXML file into an XML file. It uses an intermediary form to represent the structure and then normal DOM or JAXB techniques are used. In case of
 * JAXB the representation classes should be provided (the <em>wbxml-stream</em> package does not contain any JAXB classes, some classes were created for a few languages only for
 * testing purposes but they are not packaged in the library).
 * </p>
 * 
 * <p>
 ...
 * <li>
 * <p>
 * <strong>-j --jaxb</strong>: Use JAXB processing (object representation) instead of normal DOM processing. As it was said JAXB classes are not part of the <em>wbxml-stream</em>
 * library.
 * </p>
 * </li>
 * <li>
 * <p>
 * <strong>-e --event</strong>: Use the event reader instead of common stream reader implementation.
 * </p>
 * </li>
 * <li>
 * <p>
 ...
 * <li>
 * <p>
 * <strong>infile</strong>: WBXML file to convert, <em>-</em> can be provided to use the standard output.</em>
 * <li>
 * <p>
 * <strong>outfile</strong>: file to write the resulting XML, <em>-</em> can be provided to use the standard input.</em>
 * </ul>
 * 
 * <h3>EXAMPLES</h3>
 * 
		...
		try {
			...
			if (!useDom) {
				...
				if (parser.getDefinition().getXmlPublicId() != null && parser.getDefinition().getXmlUriRef() != null) {
					...
					marshaller.setProperty("com.sun.xml.internal.bind.xmlHeaders", String.format("\n<!DOCTYPE %s PUBLIC \"%s\" \"%s\">",
							jc.createJAXBIntrospector().getElementName(obj), parser.getDefinition().getXmlPublicId(), parser.getDefinition().getXmlUriRef()));
				}
