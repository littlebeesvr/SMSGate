...
/**
 * <p>
 * A extension in the WBXML specification is any string that can be used inside attribute values and tag PCDATA to compact the value. For example if a extension is defined for
 * "ENABLED" and attribute value <em>status="ENABLED"</em> can be compacted for the extension and if the value is <em>status="PARTIALLYENABLED"</em> the value is partially
 * substituted (PARTIALLY and the extension). In the definition the extension is just a mb_u_int32 token which represents a string value. In the specification extensions are
 * explained in chapter <em>5.8.4.2. Global Extension Tokens</em>. There there are three types of extensions but only the EXT_T is used.
 * </p>
 * 
 * <p>
...
public class WbXmlExtensionDef implements Comparable<WbXmlExtensionDef> {
	...
	public String toString() {
		return new StringBuilder(this.getClass().getName()).append(": ").append(value).append("->").append(WbXmlLiterals.formatUInt8(token))
