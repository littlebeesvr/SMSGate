 ...
 *
 * <p>
 * WBXML is a format that assigns several identifiers to different elements of an XML (tags, attributes, values,...). This way when you try to parse/encode any WBXML document you
 * need to know the language you are using. That language specification gives you those identifiers. It is impossible to parse/encode a unknown language specification. This class
 * represents a Java representation of any WBXML language definition. This definition is based on a properties file which needs to be provided
 * </p>
 * 
 * <p>
 * Basically a language definition is the following list of information (some are optional fields):
 * </p>
 * 
 * <ul>
 * <li>name: A simple name to classify the language (for example SyncML 1.1).</li>
 * <li>publicId: WBXML format defines that each language should have a identifier (numeric). 0x01 represents the unknown language (it does not have a standard id).</li>
 * <li>Formal public Id: The formal name of the XML identifier, any XML definition has a FPI (for example SyncML version 1.1 is <em>-//SYNCML//DTD SyncML 1.1//EN</em>). This FPI is
 * used in any DOCTYPE.</li>
 * <li>XML URI definition: The URL where the XMl definition for the language can be obtained (again it appears in any DOCTYPE).</li>
 * <li>Root element: The root element for the language (in SyncML "SyncML" tag).</li>
 * <li>JAXB class: This optional field should be passed if the definition has JAXB classes created for the language (using xjc for example).</li>
 * <li>Namespaces: The list of namespaces the language manages (a namespace is the uri and the prefix).</li>
 * <li>Tags: List of tags definition for the language (with the respective token identifiers).</li>
 ...
 * <li>Linked definitions: This is a non standard feature. Some definitions mix some languages using opaques. This way definitions can have another linked ones. The
 * parsing/encoding methods use all of them.</li>
 * </ul>
 * 
 * <p>
 * The properties file is parsed and an object of this kind is created in order to parse or encode a WBXML document of this language. If the language is not specified no WBXML
 * document can be parsed or encoded.
 * </p>
 * 
 * @author ricky
 * @see WbXmlTagDef
...
public class WbXmlDefinition {
	...
	/**
	 * Attributes keyed by the prefixed name. Cos an attribute can be represented by several tokens (it depends on the value) a set is used.
	 */
	 ...
	 * @param publicId
	 *            The WBXML standard identifier for the language
	 */
	 ...
	 *            The language name
	 * @param publicId
	 *            The WBXML identifier
	 * @param xmlPublicId
	 *            The XML formal identifier
	 */
	 ...
	 *            DTD uri for the xml
	 * @param clazz
	 *            The JAXB class that encapsulates the root element
	 */
	 ...
	 * 
	 * @param name
	 *            The name of the linked def to get
	...
	/**
	 * Add a new namespace to the list of namespaces managed by this language definition. The methos adds it to both maps.
	 * 
	 * @param ns
	...
	/**
	 * Add a new extension to the language definition. The method adds it to both maps.
	 * 
	 * @param ext
	 *            The new extension.
	 */
	...
	/**
	 * Add a new opaque plugin for attributes for this language definition. The method adds it to both maps.
	 * 
	 * @param attr
	 *            The attribute with a plugin associated
	 * @param plugin
	 *            The plugin
	 */
	 ...
	 * 
	 * @param tag
	 *            The tag with a plugin associated
	 * @param plugin
	 *            The plugin
	 */
	...
	/**
	 * Add a nerw Attribute definition to this language definition. The method adds it to both maps.
	 * 
	 * @param attr
	...
	/**
	 * Add a new attribute value for this language definition. The method adds it to both maps.
	 * 
	 * @param attrVal
	...
	/**
	 * Method that compacts a attribute value using extensions and attribute values. In WBXML the normal string attribute value can be compacted using extensions or attribute
	 * values, this method search over the definition to compact a normal string value into a list of them that matches with a attribute value or a extension.
	 * 
	 * @param value
	...
	/**
	 * Private method that check if a extension or attribute value is found in the value passed. If it is contained the string is splitted in three possible parts (the part before
	 * the match, the matched string, and the rest). Obviously the returned list is from one to three sized list. If there is no match a null is returned.
	 * 
	 * @param value
	 *            The value to search the string in
	 * @param valueToFound
	 *            The value (correspond from an extension or attribute value) to look for
	 * @param start
	 *            The start point to search (attribute values can start after 0).
	 * @param attr
	 *            true if attr, false if extension
	...
	/**
	 * Recursive methodthe that searches for possible compaction matches of an attribute value against the definition (attribute values or extensions can be used to compact it).
	 * This method searches in a specified order, first attribute values and then extension. In both larger definitions are taken into account first.
	 * 
	 * @param value
	 *            The attribute value to compact
	 * @param start
	 *            The start (cos an attribute can define a part of a value)
	...
	/**
	 * Similar method of the recursive version of compactAttributeValue. This method searches over the extensions in order to compact the value in a list of defined extensions.
	 * 
	 * @param value
	 *            The value to be compacted
	 * @param start
	...
	/**
	 * Method that locates an attribute in the language definition. WBXML let define several tokens for the same attribute (different values), so a set is returned.
	 * 
	 * @param name
	...
	/**
	 * Locate the attribute using the attribute name and the value. Cos the set is a TreeSet the better match is assured (the longer value, the better).
	 * 
	 * @param name
	 *            The prefixed attribute name
	 * @param value
	...
	public WbXmlAttributeDef locateAttribute(String name, String value) {
		...
		if (attrs != null) {
			for (WbXmlAttributeDef attr : attrs) {
				if (name.equals(attr.getNameWithPrefix()) && (attr.getValue() == null || value.startsWith(attr.getValue()))) {
					return attr;
				}
			}
		}
	 ...
	 *            The page code
	 * @param token
	 *            The attribute value token byte
	 * @return The attribute value definition or null
	 */
	...
	/**
	 * Method that locates an extension using the extension code. TODO: The WBXML format defined a mb_u_int32 the extension but libwbxml uses a single byte, maybe any language uses
	 * more than 255 extensions.
	 * 
	 * @param ext
	 *            The extension code
	...
	/**
	 * Method that searches over the linked definitions in order the definition that handles a specified tag. It does not return current definition.
	 * 
	 * @param name
	 *            The prefixed name of the tag
	 ...
	 * 
	 * @param name
	 *            The prefixed name of the attribute
	...
	/**
	 * Method that obtains the prefix of a namespace uri. Take in mind that the definition needs prefixed names if the language uses namespaces.
	 * 
	 * @param namespaceURI
	 *            The namespace uri
	...
	/**
	 * Utility method to obtain a prefix from all the possible definitions in this definition. The method used all the definitions (the main one and the linked).
	 * 
	 * @param namespaceURI
	 *            The namespaceURI to locate a prefix from
	...
	/**
	 * Utility method to obtain a namespaceURI from all the possible definitions used in this definition. The method uses all the language definitions includying the linked ones.
	 * 
	 * @param prefix
	 *            The prefix of the namespaceURI to search
