...
/**
 * <p>
 * The initialization class is a way to load into the JVM all the language definitions that the implementation is going to use. The class is maybe a bit overelaborated.
 * </p>
 * 
 * <p>
 * The class loads all the definitions that are paced in a specified classpath location:
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * Remember that the languages are loaded from the classpath, not from a directory in the file system. Only JAR or normal directory can be used to place the property files.
 * </p>
 * 
 * <p>
 * All the definition properties file should be of the name <em>wbxml.&lt;LANG_NAME&gt;.properties</em> and they are just loaded into the system. Then all the languages can be
...
public class WbXmlInitialization {
	...
	/**
	 * The system property name for changing default classpath location of the language definition properties file,
	 */
	...
	/**
	 * Name for the property that has the publid id of the definition. It should be 0x01 (unknown) if the definitions has no standard name.
	 */
	static public final String PROP_WBXML_PUBLIC_ID = "wbxml.publicid";

	/**
	 * Name for the property that has the FPI of the definition. It can be omitted if the definition has no formal id but it that case the recognition of the language can be
	 * impossible and parsing/decoding needs direct specification of the definition language to use.
	 */
	static public final String PROP_WBXML_XML_PUBLIC_IDENTIFIER = "wbxml.xmlpublicidentifier";

	/**
	 * Name for the property that has the URI for dtd files of the definition. It can be null.
	 */
	static public final String PROP_WBXML_XML_URI_REFERENCE = "wbxml.xmlurireference";

	/**
	 * Name for the property that specifies the root element of the language. Please set it with some value (in case the definition has several possible root element just put the
	 * most common one). This should be prefixed if namespaces are used.
	 */
	static public final String PROP_WBXML_ROOT_ELEMENT = "wbxml.rootelement";

	/**
	 * The name for the property that specifies the JAXB class for the root element. This can be null. In the implementation no JAXB classes are packaged inside the library. In the
	 * test part there are some of them just for testing purposes.
	 */
	static public final String PROP_WBXML_CLASS_ELEMENT = "wbxml.class";

	/**
	 * Prefix for properties that handle the language tags. The tags are in the following form:
	 * 
	 * <pre>
	...
	/**
	 * Prefix for properties that defines the namespaces used in the definition. There are a lot of languages that do not use them and in case they are used, all the tags should be
	 * prefixed. The format is the following:
	 * 
	 * <pre>
	...
	/**
	 * Prefix for properties that handle the language attributes. The attributes can define part of the value so the format is a bit different and it is based in two properties.
	 * The first one is used to specify the page and the numeric token and the second one (optional) if some value is used.
	 * 
	 * <pre>
	...
	/**
	 * Prefix for properties that handle attribute values. The values are common values for attributes that has a specified token in order to save space in the binary WBXML
	 * representation. The format uses again two properties:
	 * 
	 * <pre>
	...
	/**
	 * Prefix for properties that handle extensions in the language. Extensions are similar to attribute values but it can be used in any string value (attribute of content). Again
	 * the format consists in two properties:
	 * 
	 * <pre>
	...
	/**
	 * Prefix used for opaques for attribute values. The opaque is a weird way of encoding any special value in a wbxml file. In this case the opaque will pase/encode special
	 * attribute values.
	 */
	static public final String PROP_WBXML_OPAQUE_ATTR_PREFIX = "wbxml.opaque.attr.";

	/**
	 * Prefix used for opaques that are used for tags. The opaque is a weird way of encoding any special value in a wbxml file. In this case the opaque will parse/encode tag
	 * contents.
	 * 
	 * <pre>
	...
	/**
	 * The suffix use when a second property is needed (attributes, extensions and so on).
	 */
	static public final String PROP_WBXML_VALUE_SUFFIX = ".value";

	/**
	 * Prefix used for properties that has the linked definitions. Linked definitions are not standard and just are used for weird languages which mix another ones (SyncML for
	 * instance). The format is the following:
	 * 
	 * <pre>
	 ...
	 * 
	 * @param key
	 *            The property key for the namespace
	 * @param value
	 *            The value for the namespace (the namespaceURI)
	...
	/**
	 * Private method that constructs the attribute using the property. As attributes use two properties the second property will be found inside the properties again.
	 * 
	 * @param props
	 *            The properties to search for the second key
	 * @param key
	 *            The property key for the attribute
	 * @param value
	 *            The value of the key (the token)
	...
	/**
	 * Private method that constructs the attribute value using the property. As attribute values use two properties the second property will be found inside the properties again.
	 * 
	 * @param props
	 *            The properties to search for the second key
	 * @param The
	 *            property key for the attribute value
	 * @param value
	 *            The value of the key (the token)
	...
	/**
	 * Private method that constructs the extension using the property. As extensions use two properties the second property will be found inside the properties again.
	 * 
	 * @param props
	 *            The properties to search for the second key
	 * @param The
	 *            property key for the extension
	 * @param value
	 *            The value of the key (the token)
	 ...
	 * 
	 * @param def
	 *            The definition to add the opaque to
	 * @param props
	 *            The properties to search the attribute the opaque is used to
	 * @param key
	 *            The property key for the opaque attr
	 * @param value
	 *            The value (class) of the property
	 */
	static private void addOpaqueAttrPlugin(WbXmlDefinition def, Properties props, String key, String value) {
		try {
			...
			String name = keys[4];
			String tagProp = new StringBuilder(PROP_WBXML_ATTR_PREFIX).append(pageCode).append(".").append(name).toString();
	 ...
	 *            The definition to add the opaque to
	 * @param props
	 *            The properties to search the tag the opaque is used to
	 * @param key
	 *            The property key for the opaque tag
	 * @param value
	 *            The value (class) of the property
	 */
	static private void addOpaqueTagPlugin(WbXmlDefinition def, Properties props, String key, String value) {
		try {
			...
			byte pageCode = Integer.decode(keys[3]).byteValue();
			String name = keys[4];
			String tagProp = new StringBuilder(PROP_WBXML_TAG_PREFIX).append(pageCode).append(".").append(name).toString();
	...
	/**
	 * Private method that adds the linked definition to the current one. This method is calling during the file processing, cos the linked definition could not be loaded yet it
	 * just store the name. Then another method will put the complete definition.
	 * 
	 * @param def
	 *            The definition which is being processed
	 * @param value
	 *            The name of the linked definition
	 */
	 ...
	 * 
	 * @param props
	 *            The properties of the definition file
	...
	static private WbXmlDefinition loadDefinition(Properties props) {
		...
		for (String key : props.stringPropertyNames()) {
			...
			} else if (key.startsWith(PROP_WBXML_ATTR_PREFIX) && !key.endsWith(PROP_WBXML_VALUE_SUFFIX)) {
				WbXmlAttributeDef attr = getAttrDefinition(props, key, props.getProperty(key));
				if (attr != null) {
					def.addAttr(attr);
				}
			} else if (key.startsWith(PROP_WBXML_ATTR_VALUE_PREFIX) && !key.endsWith(PROP_WBXML_VALUE_SUFFIX)) {
				WbXmlAttributeValueDef attrVal = getAttrValueDefinition(props, key, props.getProperty(key));
				if (attrVal != null) {
					def.addAttrValue(attrVal);
				}
			} else if (key.startsWith(PROP_WBXML_EXT_PREFIX) && !key.endsWith(PROP_WBXML_VALUE_SUFFIX)) {
	 ...
	 * 
	 * @param resource
	 *            The url resource that contains the JAR
	 * @param path
	 *            The path defined to contain the definitions
	 */
	static private void loadPropertiesJar(URL resource, String path) {
		...
		try {
			...
			while (e.hasMoreElements()) {
				...
				if (entry.getName().startsWith(path + "/" + PROPERTIES_PREFIX) && entry.getName().endsWith(PROPERTIES_SUFFIX)) {
	 ...
	 * 
	 * @param file
	 *            The directory that contains the definitions
	 */
	...
	/**
	 * Private method that does the post-processing part for definitions. This method iterates all the definitions and, in case linked definitions are defined, the real definition
	 * is placed in there.
	 */
	...
	/**
	 * Private method that does the initialization and loading of all the definition languages. The property files can be placed in the classpath (using a JAR file or a common
	 * directory).
	 */
