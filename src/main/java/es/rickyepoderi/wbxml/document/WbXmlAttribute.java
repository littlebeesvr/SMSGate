 ...
 * 
 * <p>
 * Java representation for an attribute in the WBXML specification. An attribute is defined in the following way:
 * </p>
 * *
 * 
 * <pre>
 ...
 * 
 * <p>
 * So the attribute is the name of the attribute (then when encoded it could be used as LITERAL or not, depending if it is defined in the specification), and a list of values.
 * Those can be an attribute value, a string (STR_I or STR_T), a extension (part of the value corresponds with an extension), and entity or a opaque data. In general the in memory
 * representation is just the name and a list of string for the values. The value can be compacted or not, compacting a value is using calling the definition method that searches
 * for attributes values and extensions to reduce the length of the string value.
 * </p>
 * 
 * @author ricky
 */
public class WbXmlAttribute {
	...
	/**
	 * The name of the attribute. This name corresponds to the one used in the definition (should be prefixed if the language uses namespaces).
	 */
	private String name = null;

	/**
	 * List of values. Any value can match a extension or an attribute value. An entity is just something like this "&20;"
	 */
	 ...
	 * 
	 * @param name
	 *            The name of the attribute (prefixed!)
	 */
	 ...
	 *            The name of the attribute (prefixed!)
	 * @param value
	 *            The value (only one in the list)
	 */
	...
	/**
	 * Method that return the name without the prefix if prefixed, if not prefixed returns the normal name.
	 * 
	...
	/**
	 * Method that joins all the values in a unique string. Simple concatenating is used.
	 * 
	...
	/**
	 * Method to compact the attribute values. This method first checks attribute values specified in the language definition and then performs the same with extensions. Methods of
	 * the definition are used to compact the string values. After the call the attribute is compacted.
	 * 
	 * @param encoder
	 *            The encoder used in the encoding process
	 * @param def
	...
	/**
	 * This method convert the list of values again into a only one value. After that the attribute is not compacted. TODO: what about entities, now are just appended
	 * "abcd...&20;...cc"
	 */
	 ...
	 * 
	 * @param ident
	 *            The ident to better understanding
