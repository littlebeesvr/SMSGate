 ...
 *
 * <p>
 * This is the most complicated opaque in the current implementation. Although specification was not studied it seems that SyncML can include inside the Data a syncml:devinf (which
 * is another language, i.e. another WBXML document) or any other languages.
 * </p>
 * <p>
 * This is really weird and also it complicates the whole process... This opaque has made change a lot of code. The linked definitions were added to a wbxml definition in order to
 * know when a language has another languages <em>linked</em>.
 * </p>
 * 
 * <p>
...
public class SyncMLDataOpaque implements OpaqueContentPlugin {
	 ...
	 * 
	 * @param encoder
	 *            The encoder doing the encoder processing
	 * @param element
	...
	/**
	 * Method that parses the opaque. It generates another WbXmlParser and re-parse the opaque data in a ByteArrayInputStream. The definitions used are appended to the original
	 * parser (this is the big difference that language has introduced).
	 * 
	 * @param parser
	 *            The parser which is processing the decoding process
	 * @param data
	 *            The data of the opaque
	 * @return The content with the another document
	 * @throws IOException
	 *             Some error decoding the other document
	 */
	@Override
