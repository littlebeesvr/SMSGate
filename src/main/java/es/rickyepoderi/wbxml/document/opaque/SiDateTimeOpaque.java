 ...
 *
 * <p>
 * The WAP Service Indication (SI) Specification describes that the %Datetime; entity should be encoded as an opaque attribute. This value is originally an ISO8601 date time string
 * value. The specification in the chapter <em>8.2.2. Encoding of %Datetime;</em> says the following:
 * </p>
 * 
 * <p>
 * %Datetime; data MUST be encoded as OPAQUE data with each number in the string represented by its 4-bit binary value. Any non-numerical characters ("T", "Z", "-", and ":") are
 * discarded. Trailing zeros (from right to left) MUST be pair-wise omitted.
 * </p>
 * 
 * <p>
 * For example, "1999-04-30T06:40:00Z" is encoded into six octets as follows:<br/>
...
public class SiDateTimeOpaque implements OpaqueAttributePlugin {
	 ...
	 * 
	 * @param array
	 *            The array representing an hexa array but using ascii
	...
	/**
	 * Utility method that transforms a hexa byte array into an ascii array. Obviously the resulting array is twice the length of the input array (a byte in hexa is represented by
	 * two digits "0-9A-F" in the ascii resulting array).
	 * 
	 * @param array
	 *            The hexa (normal) byte array to convert in ascii
	...
	static public char[] hexArrayToCharArray(byte[] array) {
		...
		for (int i = array.length - 1; i >= 0; i--) {
			...
			char c = (b < 10) ? (char) ((byte) ('0' + b)) : (char) ((byte) ('A' + b - 10));
			buffer[i * 2 + 1] = c;
			b = (byte) (((array[i] & 0xF0) >> 4) % 16);
			c = (b < 10) ? (char) ((byte) ('0' + b)) : (char) ((byte) ('A' + b - 10));
			buffer[i * 2] = c;
		}
		return buffer;
	}
	...
	/**
	 * Method to encode the date time as specified in the SI specification. The string is cleaned from non digit values and then passed to hex ascii values.
	 * 
	 * @param encoder
	 *            The encoder which is performing the encoding process
	 * @param element
	 *            The element which attribute is being encoded
	 * @param attr
	 *            The attribute which value is being encoded
	 * @param value
	 *            The value (it should be a string content)
	 * @throws IOException
	...
	/**
	 * Method that parses the binary data to reconstruct the ISO8601 date time string. The method just gets the bunch of bytes and reconstryct the string.
	 * 
	 * @param parser
	 *            The parser which is performing the decoding/parsing process
	 * @param data
	 *            The data read from the opaque value
	...
	public String parse(WbXmlParser parser, byte[] data) throws IOException {
		...
		return new StringBuilder(year).append("-").append(month).append("-").append(day).append("T").append(hour).append(":").append(minute).append(":").append(second).append("Z")
				.toString();
	}
