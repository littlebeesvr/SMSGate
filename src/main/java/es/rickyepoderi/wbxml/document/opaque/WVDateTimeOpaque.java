 ...
 *
 * <p>
 * The WV language encodes using opaque data the DateTime tags. The problem is the datetime can be opaque or not (normal string). In theory the string is a ISO8601 DateTime string
 * but in the binary format it has a quite strange final byte. For this reason my plugin do not encode as opaque the DateTimes (it just go through normal string processing) but I
 * try to parse/decode opaque data in documents.
 * </p>
 * 
 * <p>
 * Date and time can be encoded as OPAQUE data or as a string as specified in [ISO8601]. For string encoding, refer to section 6.4 String on page 29.
 * </p>
 * 
 * <p>
 * OPAQUE encoding: the OPAQUE global token (0xC3) is followed by an mb_u_int32 that contains the length of the date bytes, then the actual bytes of the encoded date. The time is
 * always in 24-hour format; there is no need to indicate a.m. or p.m.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * This takes exactly 6 bytes (48 bits). Example: The date and time at 16:58:59 on the 25th of September in 2001 in time zone ‘Z’ would be encoded as: 0x06 0x1F 0x46 0x73 0x0E 0xBB
 * 0x5A
 * </p>
 * 
 * @author ricky
...
public class WVDateTimeOpaque implements OpaqueContentPlugin {
	...
	/**
	 * Parse the data getting the bytes as specified in the WV documentation. The letter is just appended at the end (it is not known if A is +01 or what.
	 * 
	 * @param data
	 *            The 6 bytes of the WV encoding
	 ...
	 * @param content
	 *            The content to add to the WBXML document
	 * @throws IOException
	 *             Some error
	 */
	@Override
	 ...
	 * 
	 * @param parser
	 *            The parse doing the parsing/decoding process
	 * @param data
	 *            The 6 bytes of the WV encoding
