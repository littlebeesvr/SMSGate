 ...
 * 
 * <p>
 * WBXML defines a string table that can be used in any parsing or encoding process. The table is used to store strings that are being referenced later in the document. All the
 * STR_T strings marks to the string table in order to get the string content of an attribute value or from a string content. The chapter <em>5.7. String Table</em> of the
 * specification explains this table.
 * </p>
 * 
 * <p>
 * The format of the table is the length of itself and the raw byte data with all the strings:
 * </p>
 * 
 * <pre>
 * strtbl = length *byte
 ...
 * 
 * <p>
 * The strings in the table are encoded using the specified document charset and terminated in 0x00 token. All the references in the document to those strings are done using the
 * starting position of the string in the byte array (for example if you want to reference the first string is 0x00).
 * </p>
 * 
 * 
 * @author ricky
...
public class WbXmlStrtbl {
	...
	/**
	 * map of string in the table indexed by its start position. This let us get the string very quickly with a index from the document.
	 */
	...
	/**
	 * This method adds a new string to the string table. First it checks if the string is already in the table, if not, the string is added and the position and size is
	 * calculated. If the encoder does not permit the use of the strtbl a exception is thrown. The encoder is marked using the setStrtblUsed() method.
	 * 
	 * @param encoder
	 *            The encoder used in the encoding process
	 * @param s
	 *            The string to add
	 * @return The position or index in which the string is set (this is the index to be used in the document).
	 * @throws IOException
	 *             The encoder does not permit the use of the strtbl
	 */
	 ...
	 * 
	 * @param idx
	 *            The idx to get the string from
	 * @return The string at that index position (not null)
	 * @throws IOException
	 *             That position does not contain a string
	 */
