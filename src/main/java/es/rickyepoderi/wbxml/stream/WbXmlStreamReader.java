...
/**
 * <p>
 * The XMLStreamReader interface allows forward, read-only access to XML. It is designed to be the lowest level and most efficient way to read XML data and the interface is
 * explained in <a href="http://docs.oracle.com/javase/6/docs/api/javax/xml/stream/XMLStreamReader.html">Java SE javadoc</a>.
 * </p>
 * 
 * <p>
 * The implementation in WBXML always creates initially a WbXmlDocument java object in memory (it can be said that first the java/memory representation is created in the
 * constructor part and then the java object reprsentation is iterated. The parser is used to implement the XMLStreamReader.
 * </p>
 * 
 * <p>
 * The iteration over the parser is maintained with a event (the current event the reader is right now) and a little ElementIndex which is just the current element the reader is
 * parsing and an index (the index marks the content inside the element we are treating). A queue of parent elements is also maintained to iterate over all the elements in document
 * parsed.
 * </p>
 * 
 * <p>
 * Implementing the WbXmlEventReader it was discovered that ATTRIBUTE event type should be returned as many times as attributes are in the element. Currently it is only returned
 * once (it was thought that). Besides it was discovered that underlaying implentations never use this event. They when START_ELEMENT is received get all the attributes using
 * getAttributeXXX methods. For that ATTRIBUTE event type is never returned. Please use START_ELEMENT instead.
 * </p>
 * 
 * @author ricky
 */
public class WbXmlStreamReader implements XMLStreamReader {
	...
	/**
	 * Namespace context that is initialized with the context used for every language definition used by the parser.
	 */
	...
	/**
	 * Little class to maintain the position of the reader. The position is the current element and the current index in the contents of that element.
	 */
	...
	/**
	 * Constructor using the parser. This is used when the parser already has parsed a stream. The constructor set the event type to START:DOCUMENT.
	 * 
	 * @param parser
	 *            The parser which already has a parsed document inside
	 */
	...
	/**
	 * Constructor using the input stream. The parser is created and the stream is parsed inside the constructor. The constructor set the event type to START:DOCUMENT.
	 * 
	 * @param is
	 *            The input stream with the WBXML document
	 * @param definition
	 *            The fixed definition to use
	 * @throws IOException
	 *             Some error parsing the stream
	 */
	...
	/**
	 * Constructor using the input stream. Created via input stream and not specifying a fixed definition.
	 * 
	 * @param is
	 *            The input stream with the WBXML document
	 * @throws IOException
	...
	/**
	 * Returns true if the cursor points to a character data event. In the WbXmlStreamReader only CHARACTERS, ENTITY_REFERENCE (no CDATA is managed).
	 * 
	...
	/**
	 * Returns true if the cursor points to a character data event that consists of all whitespace. NOTE: XMLChar.isSpace(char) is used which is a internal method.
	 * 
	...
	/**
	 * Returns a boolean which indicates if this attribute was created by default. It returns true if current element has an attribute at this index
	 * 
	 * @param index
	...
	/**
	 * Private method that searches the next element inside the current element. If the index is null we are right now in the start element and the next event could be attributes
	 * of the first content. If the index is not null we advance in the next content. That content could be a string, entity, pi or another element. If the content is an element
	 * the parent is added to the queue and start element is returned. If it is the other possibilities just the correspondent event is treated.
	 * 
	 * @param attributes
	 *            boolean that marks if we want to read attributes or not
	...
	private int nextInElement(boolean attributes) {
		if (attributes && !elementIndex.currentElement.isAttributesEmpty()) {
			...
			event = ATTRIBUTE;
		} else if (elementIndex.index != null && elementIndex.currentElement.contentsSize() > elementIndex.index) {
	...
	/**
	 * Get next parsing event - a processor may return all contiguous character data in a single chunk, or it may split it into several chunks. If the property
	 * javax.xml.stream.isCoalescing is set to true element content must be coalesced and only one CHARACTERS event must be returned for contiguous element content or CDATA
	 * Sections. By default entity references must be expanded and reported transparently to the application. An exception will be thrown if an entity reference cannot be expanded.
	...
	/**
	 * Get the value of a feature/property from the underlying implementation. NOTE: always returns null cos no properties defined til the moment
	 * 
	 * @param name
	...
	/**
	 * Test if the current event is of the given type and if the namespace and name match the current namespace and name of the current event. If the namespaceURI is null it is not
	...
	/**
	 * <p>
	 * * Reads the content of a text-only element, an exception is thrown if this is not a text-only element. Regardless of value of javax.xml.stream.isCoalescing this method
	...
	/**
	 * <p>
	 * Skips any white space (isWhiteSpace() returns true), COMMENT, or PROCESSING_INSTRUCTION, until a START_ELEMENT or END_ELEMENT is reached. If other than white space
	 * characters, COMMENT, PROCESSING_INSTRUCTION, START_ELEMENT, END_ELEMENT are encountered, an exception is thrown. This method should be used when processing element-only
	 * content seperated by white space.
	 * </p>
	 * <p>
	 * Precondition: none
	 * </p>
	...
	/**
	 * Returns true if there are more parsing events and false if there are no more events. This method will return false if the current state of the XMLStreamReader is
	 * END_DOCUMENT
	 * 
	...
	/**
	 * Return the uri for the given prefix. The uri returned depends on the current state of the processor. In this stream reader implementation all the prefixes for the languages
	 * used in the parser are added into the NamespaceContext property.
	 * 
	 * <p>
	...
	/**
	 * Returns the count of attributes on this START_ELEMENT, this method is only valid on a START_ELEMENT or ATTRIBUTE. This count excludes namespace definitions. Attribute
	...
	/**
	 * Returns the XML type of the attribute at the provided index NOTE. Always returns the "" empty type
	 * 
	 * @param i
	 *            the position of the attribute
	 ...
	 * 
	 * <p>
	 * This implementation just returns the namespaces for the root elements in each namespace used. The idea is the root element defines all the elements in the language
	 * definition.
	 * </p>
	 * 
	...
	/**
	 * Returns the prefix for the namespace declared at the index. Returns null if this is the default namespace declaration. As it is said only root language definition element
	 * has namespaces. Always return null.
	 * 
	 * @param i
	...
	/**
	 * Returns the uri for the namespace declared at the index. As it is said only root elements of the language definitions used by the parser has the namespaces defined. Always
	 * return null.
	 * 
	 * @param i
	 *            the position of the namespace declaration
	...
	/**
	 * <p>
	 * Returns the current value of the parse event as a string, this returns the string value of a CHARACTERS event, returns the value of a COMMENT, the replacement value for an
	 * ENTITY_REFERENCE, the string value of a CDATA section, the string value for a SPACE event, or the String value of the internal subset of the DTD. If an ENTITY_REFERENCE has
	 * been resolved, any character data will be reported as CHARACTERS events.
	 * </p>
	 * 
	...
	/**
	 * Returns an array which contains the characters from this event. This array should be treated as read-only and transient. I.e. the array will contain the text characters
	 * until the XMLStreamReader moves on to the next event. Attempts to hold onto the character array beyond that time or modify the contents of the array are breaches of the
	...
	/**
	 * Gets the the text associated with a CHARACTERS, SPACE or CDATA event. Text starting a "sourceStart" is copied into "target" starting at "targetStart". Up to "length"
	 * characters are copied. The number of characters actually copied is returned. The "sourceStart" argument must be greater or equal to 0 and less than or equal to the number of
	 * characters associated with the event. Usually, one requests text starting at a "sourceStart" of 0. If the number of characters actually copied is less than the "length",
	 * then there is no more text. Otherwise, subsequent calls need to be made until all text has been retrieved. For example: int length = 1024; char[] myBuffer = new char[ length
	 * ]; for ( int sourceStart = 0 ; ; sourceStart += length ) { int nCopied = stream.getTextCharacters( sourceStart, myBuffer, 0, length ); if (nCopied < length) break; }
	...
	/**
	 * Return the current location of the processor. If the Location is unknown the processor should return an implementation of Location that returns -1 for the location and null
	 * for the publicId and systemId. The location information is only valid until next() is called
	 * 
	...
	/**
	 * Returns the (local) name of the current event. For START_ELEMENT or END_ELEMENT returns the (local) name of the current element. For ENTITY_REFERENCE it returns entity name.
	 * The current event must be START_ELEMENT or END_ELEMENT, or ENTITY_REFERENCE
	 * 
	...
	/**
	 * the current event is a START_ELEMENT or END_ELEMENT this method returns the URI of the prefix or the default namespace. Returns null if the event does not have a prefix.
	 * 
	...
	/**
	 * Get the xml version declared on the xml declaration Returns null if none was declared. NOTE: Always returns 1.0
	 * 
	...
	/**
	 * Get the standalone declaration from the xml declaration. It always return false.
	 * 
	...
	/**
	 * Checks if standalone was set in the document. It always return false.
	 * 
	 * @return true if standalone was set in the document, or false otherwise
	 */
	@Override
	...
	/**
	 * Get the target of a processing instruction. NOTE: Unsupported, it throws and exception
	 * 
