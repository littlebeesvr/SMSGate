...
/**
 * <p>
 * The XMLStreamWriter interface specifies how to write XML. The XMLStreamWriter does not perform well formedness checking on its input. However the writeCharacters method is
 * required to escape & , &lt; and &gt;. For attribute values the writeAttribute method will escape the above characters plus " to ensure that all character content and attribute
 * values are well formed. Each NAMESPACE and ATTRIBUTE must be individually written. The interface is explained in
 * <a href="http://docs.oracle.com/javase/6/docs/api/javax/xml/stream/XMLStreamWriter.html">XMLStreamWriter</a>.
 * </p>
 * 
 * <p>
 * In the WbXml library this stream writer constructs while writing the memory representation of the WbXmlDocument object (all the elements, attributes, contents and so on) and in
 * the final writeEndDocument() the document is encoded in the real output stream. So only in-memory processing is done until the final method.
 * </p>
 * 
 * <p>
 * Currently the output stream has two possibilities:
 * </p>
 * 
 * <ul>
 * <li>encoderType: This encoder is the same that the WbXmlEncoder uses. This means the encoding should be configured to not use the strtbl (NO), only use it if needed (ID_NEEDED)
 * or always (ALWAYS). By default it is configured IF_NEEDED.</li>
 * <li>skipSpaces: if true values which are completely blank spaces are not considered, and real values are trimmed (start and end). if false all contents are written to the WBXML.
 * "true" by default.</li>
 * </ul>
 * 
...
public class WbXmlStreamWriter implements XMLStreamWriter {
	...
	/**
	 * Constructor using all the input values: output stream, language definition, type of encoding and boolean to set skip spaces or not and the encoding.
	 * 
	 * @param os
	 *            The Ouput Stream to write the WBXML to
	 * @param def
	 *            The language definition to use
	 * @param encoderType
	 *            The type of encoding to perform (strtbl use)
	 * @param skipSpaces
	 *            The parser skip spaces or consider them
	 * @param encoding
	 *            Encoding to use when not specified in start document
	 */
	...
	/**
	 * Constructor using all the input values: output stream, language definition, type of encoding and boolean to set skip spaces or not.
	 * 
	 * @param os
	 *            The Ouput Stream to write the WBXML to
	 * @param def
	...
	/**
	 * Constructor with only stream and language definition. Encoder type is IF_NEEDED and spaces are skipped.
	 * 
	 * @param os
	 *            The ouput stream to write the WBXML to
	 * @param def
	...
	/**
	 * Constructor only using the ouput stream. The language is guessed using the root element or DOCTYPE definition.
	 * 
	 * @param os
	 *            The language definition to use
	 */
	...
	/**
	 * Write the XML Declaration. Defaults the XML version to 1.0, and the encoding to utf-8. In WBXML stream the version is always set to 1.3 (other versions are not supported).
	 * 
	 * @param encoding
	...
	/**
	 * Writes a start tag to the output. All writeStartElement methods open a new scope in the internal namespace context. Writing the corresponding EndElement causes the scope to
	 * be closed
	 * 
	 * @param localName
	...
	/**
	 * Writes a start tag to the output. This is the only method implemented of this king (all other versions finally call this one). Different implementations uses this method
	 * differently, so it is thought very versatile. The prefix or namespace of the tag is obtained from the arguments (prefix or namespace) and if they are not passed is inherited
	 * from the parent element.If the caller later call writeNamespace then the namespace is reassigned. The element is not written here (cos later namespaces could be set), it is
	 * written in the end element.
	 * 
	 * @param prefix
	 *            local name of the tag, may not be null
	 * @param localName
	...
	/**
	 * Writes an empty element tag to the output. Not implemented (cos if there are namespaces it does not work).
	 * 
	 * @param prefix
	 *            the prefix of the tag, may not be null
	 * @param localName
	...
	/**
	 * Calculates the tag based on current nsctx and advance the current position to the parent element if parents exist.
	 */
	...
	/**
	 * Writes an end tag to the output relying on the internal state of the writer to determine the prefix and local name of the event. In the current implementation it completes
	 * the current element with proper prefix and localName (cos now all the namespaces are correctly set).
	 * 
	 * @throws XMLStreamException
	 */
	@Override
	...
	/**
	 * Closes any start tags and writes corresponding end tags. In the WBXML implementation the method also performs the real encoding of the WbXmlDocument object (all the previous
	 * staff just construct in memory/Java representation object).
	 * 
	 * @throws XMLStreamException
	 *             Some error performing the real encode
	 */
	@Override
	...
	/**
	 * Write any cached data to the underlying output mechanism. The method only flush the real stream if encoded.
	 * 
	 * @throws XMLStreamException
	 */
	@Override
	...
	/**
	 * Writes an attribute to the output stream. The real method that all the other variants call. As in writeStartElement it tries to be very versatile cos different
	 * implementations (jaxb, transformer,...) uses it quite differently.
	 * 
	 * @param prefix
	...
	/**
	 * Writes a namespace to the output stream If the prefix argument to this method is the empty string, "xmlns", or null this method will delegate to writeDefaultNamespace. In
	 * the WBXML implementation the prefix of the current element is updated to the one passed.
	 * 
	 * @param prefix
	...
	/**
	 * Writes a processing instruction. NOTE: Not implemented, throws an exception
	 * 
	 * @param target
	...
	/**
	 * Write a DTD section. This string represents the entire doctypedecl production from the XML 1.0 specification. Nothing is really written just it is used to guess the language
	 * definition if it was not passed in the constructor.
	 * 
	 * @param dtd
	 ...
	 * 
	 * @param data
	 *            The data to skip spaces from the beginning
	 ...
	 * 
	 * @param data
	 *            The data to skip spaces from the end
	 * @return The data with the whitespaces skipped
	 */
	...
	/**
	 * Private method that trims whitespaces from the beginning and the end. It calls to the other two methods.
	 * 
	 * @param data
	 *            The data to skip spaces
	 * @return The data with the whitespaces skipped
	 */
	 ...
	 *            The test to write
	 * @param skip
	 *            true if whitespaces should be skipped, false if not
	 */
	...
	/**
	 * Sets the prefix the uri is bound to. This prefix is bound in the scope of the current START_ELEMENT / END_ELEMENT pair. If this method is called before a START_ELEMENT has
	 * been written the prefix is bound in the root scope.
	 * 
	 * @param prefix
	...
	/**
	 * Binds a URI to the default namespace This URI is bound in the scope of the current START_ELEMENT / END_ELEMENT pair. If this method is called before a START_ELEMENT has been
	...
	/**
	 * Sets the current namespace context for prefix and uri bindings. This context becomes the root namespace context for writing and will replace the current root namespace
	 * context. Subsequent calls to setPrefix and setDefaultNamespace will bind namespaces using the context passed to the method as the root context for resolving namespaces. This
	 * method may only be called once at the start of the document. It does not cause the namespaces to be declared. If a namespace URI to prefix mapping is found in the namespace
	 * context it is treated as declared and the prefix may be used by the StreamWriter.
	 * 
	 * @param nc
	...
	/**
	 * Get the value of a feature/property from the underlying implementation NOTE: Not implemented, it throws an exception
	 * 
	 * @param string
