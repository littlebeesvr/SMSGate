...
/**
 * <p>
 * Class that implements the XMlEventReader for the WbXML format. It uses the WbXmlStreamReader internally, all the events are constructed using the stream implementation and
 * methods.
 * </p>
 * 
 * <p>
 * One thing is not completely understood. The ATTRIBUTE event in the WbXmlStreamReader is just reported one (the reader is at ATTRIBUTE). But here it seems that the event should
 * be reported for any attributes, one by one. That means that Stream implementation is wrong. Checking this I saw that the ATTRIBUTE event is never used in any Java parsing. It
 * seems that the parsing process reads the START_ELEMENT and from it it gets all the attributes (both using Stream or Event).
 * </p>
 * 
 * <p>
 * The summary is that ATTRIBUTE event is never returned by both implementations. The attributes should be read from the START_ELEMENT. If someone is sure how the ATTRIBUTE event
 * should work in both readers please comment something.
 * </p>
 * 
 * <p>
 * It was deeply considered to just implement this reader as a class that extends XMLStreamReaderImpl. This class is the internal class in javaSE that implements XMLEventReader.
 * There is a constructor which uses a XMLStreamReader, so it could have been implemented just using that class and creating it using a WbXmlStreamReader. Some tests were done and
 * it was perfectly functional. Besides that solution would have been saved the implementation of any event. But finally it was decided to implement the real WbXmlEventReader to
 * avoid being dependant of internal JavaSE classes
 * </p>
 * 
 * @author ricky
 */
public class WbXmlEventReader implements XMLEventReader {
	 ...
	 * 
	 * @param stream
	 *            The stream reader not modified after creation.
	 */
	 ...
	 * 
	 * @param parser
	 *            The parser which already has parsed a WBXML file
	 ...
	 * 
	 * @param is
	 *            The InputStream to read the WBXML
	 * @throws IOException
	 *             Some exception with the file
	 * @throws XMLStreamException
	 *             Some exception with the parsing
	 */
	 ...
	 *            The InputStream to read the WBXML
	 * @param definition
	 *            The definition to use (forced). If null it is guessed.
	 * @throws IOException
	 *             Some exception with the file
	 * @throws XMLStreamException
	 *             Some exception with the parsing
	 ...
	 * 
	 * @param eventType
	 *            The type event of the next event
	 * @param stream
	 *            The stream at the correct point of reading
	...
	/**
	 * Check the next XMLEvent without reading it from the stream. Returns null if the stream is at EOF or has no more XMLEvents. A call to peek() will be equal to the next return
	 * of next().
	 * 
	...
	/**
	 * Skips any insignificant space events until a START_ELEMENT or END_ELEMENT is reached. If anything other than space characters are encountered, an exception is thrown. This
	 * method should be used when processing element-only content because the parser is not able to recognize ignorable whitespace if the DTD is missing or not interpreted.
	 * 
	...
	/**
	 * Removes from the underlying collection the last element returned by the iterator (optional operation). This method can be called only once per call to next. The behavior of
	 * an iterator is unspecified if the underlying collection is modified while the iteration is in progress in any way other than by calling this method.
	 */
	@Override
	public void remove() {
