...
/**
 * <p>
 * Implementation of the StartElement event in the wbxml-stream library.
 * </p>
 * 
 * @author ricky
 */
public class WbXmlStartElementEvent extends WbXmlEvent implements StartElement {
	...
	/**
	 * Get the name of this event. It is obtained from the WbXmlElement and the definition.
	 * 
	...
	/**
	 * Returns an Iterator of non-namespace declared attributes declared on this START_ELEMENT, returns an empty iterator if there are no attributes. The iterator must contain only
	 * implementations of the javax.xml.stream.Attribute interface. Attributes are fundamentally unordered and may not be reported in any order.
	 * 
	...
	/**
	 * Returns an Iterator of namespaces declared on this element. This Iterator does not contain previously declared namespaces unless they appear on the current START_ELEMENT.
	 * Therefore this list may contain redeclared namespaces and duplicate namespace declarations. Use the getNamespaceContext() method to get the current context of namespace
	 * declarations. The iterator must contain only implementations of the javax.xml.stream.Namespace interface. A Namespace isA Attribute. One can iterate over a list of
	 * namespaces as a list of attributes. However this method returns only the list of namespaces declared on this START_ELEMENT and does not include the attributes declared on
	 * this START_ELEMENT. Returns an empty iterator if there are no namespaces. This implementation always return empty list cos the elements are always prefixed and with
	 * namespaces defined.
	 * 
	...
	/**
	 * Gets a read-only namespace context. If no context is available this method will return an empty namespace context. The NamespaceContext contains information about all
	 * namespaces in scope for this StartElement.
	 * 
	...
	public String toString() {
		return new StringBuilder("StartElement: ").append(this.element).toString();
	}
}
