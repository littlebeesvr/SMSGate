...
public class SgipBindRequestMessage extends DefaultMessage {
	...
	/**
	 * 登录类型。 1：SP向SMG建立的连接，用于发送命令 2：SMG向SP建立的连接，用于发送命令 3：SMG之间建立的连接，用于转发命令 4：SMG向GNS建立的连接，用于路由表的检索和维护 5：GNS向SMG建立的连接，用于路由表的更新 6：主备GNS之间建立的连接，用于主备路由表的一致性
	 * 11：SP与SMG以及SMG之间建立的测试连接，用于跟踪测试 其它：保留
	 */
	...
	public String toString() {
		return String.format("BindRequestMessage [loginType=%s, loginName=%s, loginPassowrd=%s, reserve=%s, header=%s]", loginType, loginName, loginPassowrd, reserve, getHeader());
	}
