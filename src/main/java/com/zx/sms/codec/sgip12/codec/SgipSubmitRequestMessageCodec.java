...
public class SgipSubmitRequestMessageCodec extends MessageToMessageCodec<Message, SgipSubmitRequestMessage> {
	...
	protected void decode(ChannelHandlerContext ctx, Message msg, List<Object> out) throws Exception {
		...
		int msgLength = LongMessageFrameHolder.getPayloadLength(requestMessage.getMsgfmt().getAlphabet(), bodyBuffer.readInt());
	...
	protected void encode(ChannelHandlerContext ctx, SgipSubmitRequestMessage requestMessage, List<Object> out) throws Exception {
		...
		ByteBuf bodyBuffer = ctx.alloc().buffer(
				SgipSubmitRequest.SPNUMBER.getBodyLength() + (requestMessage.getUsercount() - 1) * SgipSubmitRequest.USERNUMBER.getLength() + requestMessage.getMessagelength());

		bodyBuffer
				.writeBytes(CMPPCommonUtil.ensureLength(requestMessage.getSpnumber().getBytes(GlobalConstance.defaultTransportCharset), SgipSubmitRequest.SPNUMBER.getLength(), 0));
		bodyBuffer.writeBytes(
				CMPPCommonUtil.ensureLength(requestMessage.getChargenumber().getBytes(GlobalConstance.defaultTransportCharset), SgipSubmitRequest.CHARGENUMBER.getLength(), 0));
		int usercount = requestMessage.getUsercount();
		bodyBuffer.writeByte(usercount);
		for (int i = 0; i < usercount; i++) {
			String[] destTermId = requestMessage.getUsernumber();
			bodyBuffer.writeBytes(CMPPCommonUtil.ensureLength(destTermId[i].getBytes(GlobalConstance.defaultTransportCharset), SgipSubmitRequest.USERNUMBER.getLength(), 0));
		}

		bodyBuffer.writeBytes(CMPPCommonUtil.ensureLength(requestMessage.getCorpid().getBytes(GlobalConstance.defaultTransportCharset), SgipSubmitRequest.CORPID.getLength(), 0));
		bodyBuffer.writeBytes(
				CMPPCommonUtil.ensureLength(requestMessage.getServicetype().getBytes(GlobalConstance.defaultTransportCharset), SgipSubmitRequest.SERVICETYPE.getLength(), 0));
		bodyBuffer.writeByte(requestMessage.getFeetype());
		bodyBuffer
				.writeBytes(CMPPCommonUtil.ensureLength(requestMessage.getFeevalue().getBytes(GlobalConstance.defaultTransportCharset), SgipSubmitRequest.FEEVALUE.getLength(), 0));
		bodyBuffer.writeBytes(
				CMPPCommonUtil.ensureLength(requestMessage.getGivenvalue().getBytes(GlobalConstance.defaultTransportCharset), SgipSubmitRequest.GIVENVALUE.getLength(), 0));
		...
		bodyBuffer.writeBytes(
				CMPPCommonUtil.ensureLength(requestMessage.getExpiretime().getBytes(GlobalConstance.defaultTransportCharset), SgipSubmitRequest.EXPIRETIME.getLength(), 0));
		bodyBuffer.writeBytes(
				CMPPCommonUtil.ensureLength(requestMessage.getScheduletime().getBytes(GlobalConstance.defaultTransportCharset), SgipSubmitRequest.SCHEDULETIME.getLength(), 0));
		...
		bodyBuffer.writeBytes(CMPPCommonUtil.ensureLength(requestMessage.getReserve().getBytes(GlobalConstance.defaultTransportCharset), SgipSubmitRequest.RESERVE.getLength(), 0));
