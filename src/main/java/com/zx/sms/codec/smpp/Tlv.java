...
public class Tlv implements Serializable {
	...
	/**
	 * Gets an "unsigned" version of this TLV's length of its value. A TLV has a 16-bit integer that is encoded as a short. Since Java only has "signed" shorts, any length > 32K
	 * would end up being a negative value. This method returns the "unsigned" version as an int.
	 * 
	...
	/**
	 * Returns the size of this TLV in bytes. Basically, its always 4 bytes plus the length of the value. Two bytes for tag and two byte for length and then the length of the value
	 * itself. Primarily used for encoding the PDU.
	 * 
	 * @return
	 */
	...
	/**
	 * Attempts to get the underlying value as a byte. If the underlying byte array cannot be converted, then an exception is thrown. For example, if the underlying byte array
	 * isn't a length of 1, then it cannot be converted to a byte.
	 * 
	 * @return The byte array value as a byte
	 * @throws TlvConvertException
	 *             Thrown if the underlying byte array cannot be converted to a byte.
	 */
	...
	/**
	 * Attempts to get the underlying value as an unsigned byte. If the underlying byte array cannot be converted, then an exception is thrown. For example, if the underlying byte
	 * array isn't a length of 1, then it cannot be converted to an unsigned byte.
	 * 
	 * @return The byte array value as an unsigned byte
	 * @throws TlvConvertException
	 *             Thrown if the underlying byte array cannot be converted to an unsigned byte.
	 */
	...
	/**
	 * Attempts to get the underlying value as a short. If the underlying byte array cannot be converted, then an exception is thrown. For example, if the underlying byte array
	 * isn't a length of 2, then it cannot be converted to a short.
	 * 
	 * @return The byte array value as a short
	 * @throws TlvConvertException
	 *             Thrown if the underlying byte array cannot be converted to a short.
	 */
	...
	/**
	 * Attempts to get the underlying value as an unsigned short. If the underlying byte array cannot be converted, then an exception is thrown. For example, if the underlying byte
	 * array isn't a length of 2, then it cannot be converted to an unsigned short.
	 * 
	 * @return The byte array value as an unsigned short
	 * @throws TlvConvertException
	 *             Thrown if the underlying byte array cannot be converted to an unsigned short.
	 */
	...
	/**
	 * Attempts to get the underlying value as an int. If the underlying byte array cannot be converted, then an exception is thrown. For example, if the underlying byte array
	 * isn't a length of 4, then it cannot be converted to an int.
	 * 
	 * @return The byte array value as an int
	 * @throws TlvConvertException
	 *             Thrown if the underlying byte array cannot be converted to an int.
	 */
	...
	/**
	 * Attempts to get the underlying value as an unsigned int. If the underlying byte array cannot be converted, then an exception is thrown. For example, if the underlying byte
	 * array isn't a length of 4, then it cannot be converted to an unsigned int.
	 * 
	 * @return The byte array value as an unsigned int
	 * @throws TlvConvertException
	 *             Thrown if the underlying byte array cannot be converted to an unsigned int.
	 */
	...
	/**
	 * Attempts to get the underlying value as a long. If the underlying byte array cannot be converted, then an exception is thrown. For example, if the underlying byte array
	 * isn't a length of 8, then it cannot be converted to a long.
	 * 
	 * @return The byte array value as a long
	 * @throws TlvConvertException
	 *             Thrown if the underlying byte array cannot be converted to a long.
	 */
	...
	/**
	 * Checks if another Tlv object is equal to this Tlv object. Two tlv objects are considered equal if they have the same tag and the same value.
	 * 
	 * @param t
