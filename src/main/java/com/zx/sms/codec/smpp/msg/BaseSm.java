...
/**
 * Base "short message" PDU as a super class for submit_sm, deliver_sm, and data_sm. Having a common base class they all inherit from makes it easier to work with requests in a
 * standard way, even though data_sm does NOT actually support all of the same parameters.
 * 
	...
	public void setShortMessage(byte[] value) throws SmppInvalidArgumentException {
		if (value != null && value.length > 255) {
			throw new SmppInvalidArgumentException(
					"A short message in a PDU can only be a max of 255 bytes [actual=" + value.length + "]; use optional parameter message_payload as an alternative");
		}
