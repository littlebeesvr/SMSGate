...
public class ByteBufUtil {
	...
	/**
	 * Read and create a new Address from a buffer. Checks if there is a minimum number of bytes readable from the buffer.
	 * 
	 * @param buffer
	 * @return
	...
	/**
	 * Writes an address to a buffer. If the address is null, this method will safely write out the SmppConstants.EMPTY_ADDRESS instance.
	 * 
	 * @param buffer
	 * @param value
	 * @throws UnrecoverablePduException
	...
	/**
	 * Reads a TLV from a buffer. This method is greedy and will read bytes even if it won't be able to successfully complete. It's assumed this method will only be called if its
	 * known ahead of time that all bytes will be available ahead of time.
	 * 
	 * @param buffer
	...
	/**
	 * Writes a C-String (null terminated) to a buffer. If the String is null this method will only write out the NULL byte (0x00) to the buffer.
	 * 
	 * @param buffer
	 * @param value
	...
	/**
	 * Reads a C-String (null terminated) from a buffer. This method will attempt to find the null byte and read all data up to and including the null byte. The returned String
	 * does not include the null byte. Will throw an exception if no null byte is found and it runs out of data in the buffer to read.
	 * 
	 * @param buffer
	 * @return
